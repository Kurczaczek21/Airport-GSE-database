...
IF (lineUz=lineLot and id=3 and fid_id=regexp_replace($2, '[0-9]{4}', '', 'g')) or (id=1 and fid_id=regexp_replace($2, '[0-9]{4}', '', 'g')) THEN
    p_DataPrzylotu := p_DataPrzylotu - INTERVAL '1 week';
    p_DataOdlotu := p_DataOdlotu - INTERVAL '1 week';

    FOR i IN 1..(p_LiczbaTygodni) LOOP
        -- Ustaw datę odlotu na dzień po dacie przylotu
        DataPrzylotu := p_DataPrzylotu + INTERVAL '1 week';
        DataOdlotu := p_DataOdlotu + INTERVAL '1 week';
...
alter function dodajcotygodniowylot(varchar, varchar, varchar, timestamp, timestamp, varchar, varchar, integer, integer) owner to postgres;
